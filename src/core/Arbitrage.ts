...
export class Arbitrage {
  ...
  async evaluateMarkets(
    ...
    return opportunities.sort((a, b) => {
      const diff = b.profit.sub(a.profit);
      // Use comparison instead of toNumber to avoid overflow
      return diff.isNegative() ? -1 : diff.isZero() ? 0 : 1;
    });
  }

  private async filterValidMarkets(
    markets: EthMarket[],
    tokenAddress: string
  ): Promise<EthMarket[]> {
  ...
  private async checkArbitrageOpportunity(
    ...
    try {
      ...
      const price1 = await market1.getTokensOut(
        WETH_ADDRESS,
        tokenAddress,
        ETHER
      );
      const price2 = await market2.getTokensOut(
        WETH_ADDRESS,
        tokenAddress,
        ETHER
      );
      ...
      if (profit.gt(this.thresholds.minProfitThreshold)) {
        ...
        const volume = await this.calculateOptimalVolume(
          buyMarket,
          sellMarket,
          tokenAddress,
          profit
  ...
  async takeCrossedMarkets(
    ...
    for (const market of markets) {
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          const transaction = await this.executeArbitrageTrade(
            market,
            currentBlock
          );
          if (transaction) {
  ...
  private async getOptimalGasPrice(blockNumber: number): Promise<BigNumber> {
    const { currentGasPrice, avgGasPrice } = await getGasPriceInfo(
      this.wallet.provider as Provider
    );
    const basePrice = currentGasPrice.gt(avgGasPrice)
      ? currentGasPrice
      : avgGasPrice;
  ...
  async submitBundleWithAdjustedGasPrice(
    ...
    logger.log(
      `Submitting bundle with adjusted gas price for block ${blockNumber}`
    );
    ...
    try {
      ...
      const competingBundlesGasPrices =
        await monitorCompetingBundlesGasPrices(blocksApi);
      ...
      if ("error" in bundleSubmission) {
        throw new Error(
          `Bundle submission failed: ${bundleSubmission.error.message}`
        );
      }
  ...
  private async calculateOptimalVolume(
    ...
    const priceImpactBuy = await buyFromMarket.getPriceImpact(
      tokenAddress,
      maxTradeSize
    );
    const priceImpactSell = await sellToMarket.getPriceImpact(
      tokenAddress,
      maxTradeSize
    );
    ...
    while (left.lt(right)) {
      ...
      if (
        expectedProfit.gt(maxExpectedProfit) &&
        expectedProfit.gte(this.thresholds.minProfitThreshold)
      ) {
...
export async function monitorCompetingBundlesGasPrices(blocksApi: {
  getRecentBlocks: () => any;
}): Promise<Array<BigNumber>> {
  logger.log("Entering monitorCompetingBundlesGasPrices");
  const recentBlocks = await blocksApi.getRecentBlocks();
  const competingBundlesGasPrices = recentBlocks.map(
    (block: { bundleGasPrice: any }) => block.bundleGasPrice
  );
  logger.log(
    `monitorCompetingBundlesGasPrices: competingBundlesGasPrices = ${competingBundlesGasPrices}`
  );
...
export async function calculateOptimalVolume(
  ...
  const buyImpact = await buyFromMarket.getPriceImpact(
    tokenAddress,
    optimalVolume
  );
  const sellImpact = await sellToMarket.getPriceImpact(
    tokenAddress,
    optimalVolume
  );

  // If price impact is too high, reduce volume
  if (buyImpact.add(sellImpact).gt(BigNumber.from(300))) {
    // 3% total impact
