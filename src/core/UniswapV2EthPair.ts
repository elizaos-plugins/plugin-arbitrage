...
export class UniswapV2EthPair implements EthMarket, MarketType {
  ...
  static buyFromMarket(
    _buyFromMarket: any,
    _sellToMarket: EthMarket,
    _tokenAddress: string,
    _profit: number
  ) {
    throw new Error("Method not implemented.");
  }
  static impactAndFeeFuncs(
    provider: JsonRpcProvider,
  ...
  static updateReservesFromResults(
    pairs: Array<UniswapV2EthPair>,
    results: Array<any>
  ): void {
  ...
  async getPriceImpact(
    ...
    const impact = tradeSize
      .mul(BigNumber.from(10000))
      .div(reserve.add(tradeSize));
  ...
  static async getUniswapMarkets(
    provider: JsonRpcProvider,
    factoryAddress: string
  ): Promise<Array<UniswapV2EthPair>> {
    ...
    const allPairsLength = await new Contract(
      factoryAddress,
      UNISWAP_FACTORY_ABI,
      provider
    ).allPairsLength();
    const totalBatches = Math.ceil(
      allPairsLength.toNumber() / UNISWAP_BATCH_SIZE
    );

    // Iterate over all pairs in batches, with consideration for the batch count limit
    for (
      let batch = 0;
      batch < Math.min(totalBatches, BATCH_COUNT_LIMIT);
      batch++
    ) {
      const startIndex = batch * UNISWAP_BATCH_SIZE;
      const endIndex = Math.min(
        startIndex + UNISWAP_BATCH_SIZE,
        allPairsLength.toNumber()
      );

      const batchPairs = await uniswapQuery.functions.getPairsByIndexRange(
        factoryAddress,
        startIndex,
        endIndex
      );
      ...
      if (
        Array.isArray(batchPairs) &&
        batchPairs.length > 0 &&
        Array.isArray(batchPairs[0])
      ) {
        batchPairs[0].forEach((pairArray) => {
          // Adjusted to access the first element of batchPairs, which is the actual array of pairs
          if (Array.isArray(pairArray) && pairArray.length === 3) {
            ...
            if (
              ...
              console.error(
                `Invalid address detected. Token0: ${token0}, Token1: ${token1}, PairAddress: ${pairAddress}`
              );
              return;
            }
            // Exclude pairs involving blacklisted tokens
            if (
              !blacklistTokens.includes(token0) &&
              !blacklistTokens.includes(token1)
            ) {
              // Process valid pairs here
              const marketPair = new UniswapV2EthPair(
                pairAddress,
                [token0, token1],
                "UniswapV2",
                token0,
  ...
  static async getUniswapMarketsByToken(
    ...
    try {
      ...
      const filteredPairs: UniswapV2EthPair[] = allPairsWithBalance.filter(
        (pair) => pair !== null
      ) as UniswapV2EthPair[];
      ...
      console.error(
        "An error occurred while getting Uniswap Markets By Token:",
        error
      );
  ...
  static async updateReserves(
    ...
    console.log(
      `Filtered pairs for arbitrage calculation: ${filteredPairsInArbitrage.length}`
    );
  ...
  async getBalance(tokenAddress: string): Promise<BigNumber> {
    ...
    if (tokenAddress === WETH_ADDRESS.toLowerCase()) {
      const wethBalance = await UniswapV2EthPair.fetchWETHBalance(
        this.provider,
        this.marketAddress,
        WETH_ADDRESS
      );
  ...
  async setReservesViaOrderedBalances(
  ...
  async setReservesViaMatchingArray(
    tokens: Array<string>,
    balances: Array<BigNumber>
  ): Promise<void> {
  ...
  async sellTokens(
    ...
    const populatedTransaction =
      await UniswapV2EthPair.uniswapInterface.populateTransaction.swap(
        amount0Out,
        amount1Out,
        recipient,
        []
  ...
  static async updateReservesWithMulticall(
    ...
    console.log(
      "Attempting to update reserves in one or more multicall batches"
    );
    ...
    for (let i = 0; i < pairs.length; i += CHUNK_SIZE) {
      ...
      const calls = chunk.map((pair) => {
        const callData = new Interface(UNISWAP_PAIR_ABI).encodeFunctionData(
          "getReserves"
        );
        return {
      ...
      try {
        ...
        chunk.forEach((pair, idx) => {
          try {
            ...
            if (result) {
              const [reserve0, reserve1] = new Interface(
                UNISWAP_PAIR_ABI
              ).decodeFunctionResult("getReserves", result) as [
                BigNumber,
                BigNumber,
                number,
              ];
    ...
    console.log(
      `Finished multicall update. Updated pairs: ${updatedPairs.length}`
    );
    return updatedPairs;
  }
}
