...
export class ArbitrageService extends Service {
  ...
  get capabilityDescription(): string {
    return "Provides arbitrage trading capabilities across DEX markets";
  }
  ...
  async initialize(runtime: IAgentRuntime): Promise<void> {
    ...
    if (!wsUrl && !rpcUrl) {
      throw new Error(
        "Missing both ARBITRAGE_ETHEREUM_WS_URL and ARBITRAGE_EVM_PROVIDER_URL envs"
      );
    }
    ...
    if (!flashbotsKey)
      throw new Error("Missing FLASHBOTS_RELAY_SIGNING_KEY env");
    ...
    if (!bundleExecutorAddress)
      throw new Error("Missing BUNDLE_EXECUTOR_ADDRESS env");
  ...
  async executeArbitrage(markets: CrossedMarketDetails[]): Promise<void> {
    ...
    return this.arbitrage.takeCrossedMarkets(
      markets,
      this.currentBlock,
      maxAttempts
    );
