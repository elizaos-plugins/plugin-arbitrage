...
describe("executeArbitrageAction", () => {
  ...
  describe("metadata", () => {
    it("should have correct name and description", () => {
      ...
      expect(executeArbitrageAction.description).toContain(
  ...
  describe("validation", () => {
    it("should validate required settings", async () => {
      ...
      const isValid = await executeArbitrageAction.validate(
        mockRuntime,
        mockMessage
      );
  ...
  describe("handler", () => {
    it("should execute arbitrage when opportunities exist", async () => {
      ...
      const result = await executeArbitrageAction.handler(
        mockRuntime,
        mockMessage
      );
      expect(result).toBe(true);
      expect(mockArbitrageService.evaluateMarkets).toHaveBeenCalled();
      expect(mockArbitrageService.executeArbitrage).toHaveBeenCalledWith(
        mockOpportunities
      );
    });
    ...
    it("should handle evaluation errors", async () => {
      (mockArbitrageService.evaluateMarkets as any).mockRejectedValue(
        new Error("Evaluation failed")
      );

      await expect(
        executeArbitrageAction.handler(mockRuntime, mockMessage)
    ...
    it("should handle execution errors", async () => {
      ...
      (mockArbitrageService.executeArbitrage as any).mockRejectedValue(
