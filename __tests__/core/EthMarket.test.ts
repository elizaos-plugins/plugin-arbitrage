...
class TestEthMarket extends EthMarket {
    constructor(marketAddress: string, tokenAddress: string, tokens: string[], protocol: any) {
    ...
    async sellTokens(tokenIn: string, tokenOut: string, amountIn: BigNumber, recipient: string): Promise<any> {
        return { hash: '0xhash' };
    }

    async sellTokensToNextMarket(tokenIn: string, tokenOut: string, amountIn: BigNumber, nextMarket: EthMarket): Promise<any> {
        return { hash: '0xhash' };
    }

    async getPriceImpact(tokenIn: string, tokenOut: string, amountIn: BigNumber): Promise<number> {
        return 0.02;
    }
...
describe('EthMarket', () => {
    ...
    it('should implement getTokensOut', async () => {
        ...
        const amountOut = await market.getTokensOut('0xtoken1', '0xtoken2', amountIn);
        expect(amountOut.toString()).toBe('200');
    });

    it('should implement sellTokens', async () => {
        const result = await market.sellTokens('0xtoken1', '0xtoken2', BigNumber.from('100'), '0xrecipient');
        expect(result.hash).toBe('0xhash');
    });

    it('should implement sellTokensToNextMarket', async () => {
        const nextMarket = new TestEthMarket('0xnext', '0xtoken3', ['0xtoken3'], {});
        const result = await market.sellTokensToNextMarket('0xtoken1', '0xtoken2', BigNumber.from('100'), nextMarket);
        expect(result.hash).toBe('0xhash');
    });

    it('should implement getPriceImpact', async () => {
        const impact = await market.getPriceImpact('0xtoken1', '0xtoken2', BigNumber.from('100'));
